...
function startHandForClient(client) {
  ...
  const seated = room.players.filter((player) => player.stack > 0 && !player.sittingOut);
  if (seated.length < 2) {
...
function startHand(room) {
  ...
  room.players.forEach((player) => {
    player.hand = player.stack > 0 && !player.sittingOut ? [room.deck.pop(), room.deck.pop()] : [];
    player.bet = 0;
...
function nextActiveIndex(room, fromIndex) {
  for (let step = 1; step <= room.players.length; step += 1) {
    ...
    if (room.players[index].stack > 0 && !room.players[index].sittingOut) return index;
  }
  return -1;
}
...
function nextActionIndex(room, fromIndex) {
  for (let step = 1; step <= room.players.length; step += 1) {
    const index = (fromIndex + step + room.players.length) % room.players.length;
    const player = room.players[index];
    if (player.hand.length && !player.folded && !player.allIn && player.stack > 0 && !player.sittingOut) return index;
  }
  return -1;
}
...
function publicState(room, viewerId) {
  return {
    ...
    players: room.players.map((player, index) => ({
      ...
      cards: player.id === viewerId || room.phase === "showdown" ? player.hand : player.hand.map(() => null)
    }))
  };
}
