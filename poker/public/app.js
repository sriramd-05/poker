...
function renderSeats() {
  ...
  for (let seatNumber = 1; seatNumber <= state.maxPlayers; seatNumber += 1) {
    ...
    const status = player.sittingOut ? "Away" : player.stack === 0 ? "Broke" : player.folded ? "Folded" : player.allIn ? "All-in" : player.connected ? "Online" : "Offline";
    seat.innerHTML = `
      <div class="seat-name">
        <span><b>${seatNumber}</b> ${escapeHtml(player.name)}${player.isOwner ? " *" : ""}</span>
        ${player.dealer ? '<span class="dealer">D</span>' : ""}
      </div>
      <div class="cards seat-cards"></div>
      <div class="seat-meta"><span>${player.stack}</span><span>${status}</span></div>
      <div class="seat-meta"><span>Bet ${player.bet}</span><span>W ${player.stats?.wins || 0}</span></div>
    `;
...
function renderBoard() {
  ...
  for (let i = 0; i < 5; i += 1) {
    ...
    el.textContent = card ? `${card.rank}${suitSymbol(card.suit)}` : "";
